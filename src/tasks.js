...
function getProjectTasks() {
  return getTasksFromStorage().filter(
    (task) => task.projectId === getCurrentProject()
  );
}
...
function listTasks() {
  ...
  if (taskArr.length === 0 && getProjects().length > 0) {
    if (completedListHeader) {
      completedListHeader.style.display = 'none';
    }

    const emptyMessage = document.createElement('p');
    emptyMessage.classList.add('empty-tasks');
    emptyMessage.textContent =
      "Looks like we don't have any tasks here yet. Add one by clicking below.";
  ...
  if (taskArr.some((task) => task.completed)) {
    if (completedListHeader) {
      completedListHeader.style.display = 'block';
    }
  } else {
    if (completedListHeader) {
      completedListHeader.style.display = 'none';
